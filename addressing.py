...
def discover_all_links(intent):
    ...
    for r_name, r_data in routers_global.items():
        as_r = router_to_as[r_name]

        for if_name, if_data in r_data["interfaces"].items():
            ngbr = if_data.get("ngbr")
            if not ngbr or ngbr not in routers_global:
                continue

            key = tuple(sorted([r_name, ngbr]))
            if key in seen:
                continue

            as_ng = router_to_as[ngbr]

            for ngbr_if, ngbr_if_data in routers_global[ngbr]["interfaces"].items():
                if ngbr_if_data.get("ngbr") == r_name:
                    link = (r_name, if_name, as_r, ngbr, ngbr_if, as_ng)
                    if as_r == as_ng:
...
def fill_ipv6_intra_as(intent):
    ...
    for asn, as_data in intent["AS"].items():
        ...
        plage[asn] = net.subnets(new_prefix=64)

    for r1, if1, as1, r2, if2, as2 in intra_links:
        subnet = next(plage[as1])
...
def choose_ebgp_range(intent):
    ...
    candidates = super_range.subnets(new_prefix=48)
