# Network configuration automation (IPv6 addressing, IGP, BGP, and policies)

This project generates **router startup configurations** from an **intent JSON** file. It automatically assigns **IPv6 addresses**, configures **IGP** (RIP or OSPF), builds an **iBGP full-mesh** inside each AS using **loopbacks**, configures **eBGP** on inter-AS links, and optionally applies **routing policies** using **communities + local-preference + route-maps**.
...
- `intent_filled.json`: the same intent, but with every missing IPv6 address filled.
